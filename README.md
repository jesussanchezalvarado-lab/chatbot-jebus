...
- `index.html` — interfaz. Guarda tus conversaciones y tu "memoria" en el `localStorage` de tu navegador (no hay base de datos ni login). La respuesta del bot aparece en vivo, palabra por palabra, y puedes detenerla a mitad de camino con el botón ■.
- `api/chat.js` — función serverless que transmite (streaming) la respuesta desde la API gratuita de Google Gemini con tu clave, para que esta nunca quede expuesta en el navegador.
- `api/remember.js` — función serverless liviana que corre en paralelo, para detectar datos nuevos y estables sobre ti y actualizar la memoria.
...
## El modelo de IA es gratuito

Usa la API de **Google Gemini**, que tiene una capa gratuita pensada para uso personal: no pide tarjeta de crédito y no cobra mientras te mantengas dentro de los límites de uso gratuitos (número de mensajes por minuto/día). Para uso personal normal no deberías notar el límite. Consigue tu clave gratis en [aistudio.google.com/apikey](https://aistudio.google.com/apikey) con tu cuenta de Google.
...
4. En "Environment Variables" agrega:
   - `GEMINI_API_KEY`: tu clave gratuita de [aistudio.google.com/apikey](https://aistudio.google.com/apikey).
...
Te pedirá vincular el proyecto y configurar `GEMINI_API_KEY` (puedes copiarla desde `.env.example` a `.env`).
